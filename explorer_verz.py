...
"""
TurtleBot LiDAR exploration controller.
...
class LidarController(Node):
    ...
    def lidar_callback(self, scan_data: LaserScan):
        ...
        CLOSE = 0.45         # normal obstacle threshold  
        ...
        if self.state == "follow":
            ...
            if front < CLOSE:
                ...
                duration = 1.5 + max(0.0, CLOSE - front) * 3.5   
    ...
    def _start_recovery(self, lin: float, ang: float, duration: float):
        ...
        if (now - self.last_recovery_time).nanoseconds < 5e9:
            self.repeat_recovery_count += 1
        else:
            self.repeat_recovery_count = 0

        self.last_recovery_time = now

        if self.repeat_recovery_count >= 3:
            ang *= -1.0
            duration *= 2.0
            self.repeat_recovery_count = 0
